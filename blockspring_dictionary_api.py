...
def _get_examples(tree):
    '''
        Return a list with all of the example sentences for this word
        on Merriam-Webster.
    '''
    all_example_sentences = []
    
    # Get the list of <li> tags that contain example sentences for this word.
    CONTAINER_XPATH = '//div[@class="example-sentences"]/div/ol/li'
    li_list = tree.xpath(CONTAINER_XPATH)
    
    # Extract all of the text within each <li> tag, including text within
    # nested tags such as <em>.
    for li_elem in li_list:
        example_sentence = ''.join(li_elem.xpath('descendant-or-self::text()'))
        if not example_sentence:
            continue

        # Some example sentences start with '<' + lower-case character and end with '>'.
        # Replace '<', '>' AND capitalize the first letter of these sentences.
        if len(example_sentence) >= 2 and example_sentence[0] == '<':
            example_sentence = example_sentence.replace('<','').replace('>','')
            example_sentence = example_sentence[0].upper() + example_sentence[1:] + '.'
        all_example_sentences.append(example_sentence)
...
def _get_info_from_merriam_webster(request):
    ...
    if not request.params.get('exclude_examples', None):
        mw_info['examples'] = _get_examples(tree)
